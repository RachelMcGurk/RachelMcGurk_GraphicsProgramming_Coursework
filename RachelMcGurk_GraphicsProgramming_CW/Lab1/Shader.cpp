...
void Shader::initGeom(const std::string& vertFile, const std::string& fragFile, const std::string& geomFile) // this methods is only used to initialise shaders with a geometry shader
{
	...
	glBindAttribLocation(program, 1, "texCoords"); // associate attribute variable with our shader program attribute (in this case attribute vec2 texCoords)
	glBindAttribLocation(program, 2, "normals"); // associate attribute variable with our shader program attribute (in this case attribute vec3 normals)
	...
	CheckShaderError(program, GL_VALIDATE_STATUS, true, "Error: Shader program not valid"); // check for error
...
void Shader::init(const std::string& vertFile, const std::string& fragFile)
{
	...
	glBindAttribLocation(program, 0, "position"); // associate attribute variable with our shader program attribute (in this case attribute vec3 position;)
	glBindAttribLocation(program, 1, "texCoord"); // associate attribute variable with our shader program attribute (in this case attribute vec2 texCoords)
	glBindAttribLocation(program, 2, "normal"); // associate attribute variable with our shader program attribute (in this case attribute vec3 normals)
...
{
	glm::mat4 mvp = camera.GetViewProjection() * transform.GetModel(); // defines the model view projection matrix
	glUniformMatrix4fv(uniforms[TRANSFORM_U], 1, GLU_FALSE, &mvp[0][0]); //updates the uniforms of each shader
}
